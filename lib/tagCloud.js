 ...
 */
var generateHTMLTag = function (tagType, attributes, tagText) {
    ...
    _.each(keys, function (key) {
        ...
        html += generateHTMLAttribute(key, value, attrTag);
    });
...
var generateHTMLAttribute = function (key, value, tagText) {
    return ' {key}="{value}"'.replace(/{key}/, key).replace(/{value}/, value.replace(/{{tag}}/g, tagText));
};
