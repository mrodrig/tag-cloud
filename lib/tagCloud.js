...
module.exports = {
    ...
    tagCloud: function (array, callback, opts) {
        try {
            ...
            if (defaultOptions.randomize) { array = _.shuffle(array); }
            var min = _.min(array, function(value) { return value.count; }).count,
                max = _.max(array, function(value) { return value.count; }).count,
                diff = max - min,
                numTagsPerBucket = Math.ceil(diff/defaultOptions.numBuckets);
            _.each(array, function (tag, indx) {
                html += generateHTMLTag('span', _.defaults({
                    class: defaultOptions.classPrefix + determineBucket(min, numTagsPerBucket, tag.count)
                }, defaultOptions.additionalAttributes), tag.tagName);
            });
